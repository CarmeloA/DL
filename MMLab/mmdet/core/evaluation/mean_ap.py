...
def eval_map(det_results,
    ...
    for i in range(num_classes):
        ...
        # print('{} num of tp:{}'.format(str(i),str(tp)))
        # print('{} num of fp:{}'.format(str(i),str(fp)))
        
        eps = np.finfo(np.float32).eps
        print(eps)
        print(num_gts[:, np.newaxis])
        print('=====================================================')
        recalls = tp / np.maximum(num_gts[:, np.newaxis], eps)
        print('recall:',recalls.shape)
...
def print_map_summary(mean_ap, results, dataset=None):
    ...
    if not isinstance(mean_ap, list):
        mean_ap = [mean_ap]
    header = ['class', 'tp', 'fp', 'gts', 'dets', 'recall', 'precision', 'ap']
    for i in range(num_scales):
        table_data = [header]
        for j in range(num_classes):
            row_data = [
                label_names[j], results[j]['tp'], results[j]['fp'], num_gts[i, j], results[j]['num_dets'],
